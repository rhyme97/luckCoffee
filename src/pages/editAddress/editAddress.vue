    ...
    <view class="editAddress">
        <form>
    	   <view>
            ...
            <view>
               <label class="radio"><radio value="是" :checked="editForm.defaultSite=='是'" @click="defaultHander"/>设为默认地址</label>
            </view>
            <view>
                <button type="default"  @click="deleteIt">删除</button>
                <button type="default"  @click="saveIt">保存</button>
            </view>
             
        </form>
          <uni-popup class="mypop" ref="popup" type="center">{{tips}}</uni-popup>
          <uni-popup class="mypop" ref="popup1" type="center">
            <view>
                <text>确定删除吗？</text>
                <view>
                    <button @click="delOk">确定</button>
...
export default {
    ...
    methods:{
    	...
    	saveIt(){
    		...
    		if (sure) {
    			...
    			if (myregular.test(this.editForm.tel)) {
    				...
    				uni.request({
    					method:'POST',
    					url:"http://rhymin.cn:3000/editAddress",
    					data:this.editForm,
    					success:res=>{
    						console.log(res)
    	...
    	delOk(){
    		uni.request({
    			method:'POST',
    			url:'http://rhymin.cn:3000/delAddress',
    			data:{siteId:this.editForm.siteId},
    			success:res=>{
    				if (res.data.tips=='success') {
                        this.tips='删除成功'
                        this.$refs.popup.open()
    ...
    onLoad: function(option) {
        this.editForm = JSON.parse(decodeURIComponent(option.item));
        console.log(this.editForm)
    }
}
</script>
